...
<script>
const DEFAULT_DAILY=[
  {id:"study",label:"8hrs Self Study",icon:"📚",fixed:true},
  {id:"questions",label:"4hrs Question Practice",icon:"✏️",fixed:true},
  {id:"exercise",label:"Exercise",icon:"💪",fixed:true},
  {id:"water",label:"3 Litres of Water",icon:"💧",fixed:true},
  {id:"nojunk",label:"Avoided Junk Food",icon:"🥗",fixed:true},
];
const DEFAULT_WEEKLY=[
  {id:"room",label:"Room Organize",icon:"🏠",fixed:true},
  {id:"sketch",label:"Sketching",icon:"🎨",fixed:true},
];
const ICONS=["📚","✏️","💪","💧","🥗","🏠","🎨","🧘","🏃","📖","🎵","🍎","💤","🧹","🌿","🎯","💻","📝","🛁","☀️"];
...
function addTask(type){
  ...
  if(type==="daily")setState({dailyTasks:[...state.dailyTasks,task],addingFor:null,newLabel:"",newIcon:"📝"});
  else setState({weeklyTasks:[...state.weeklyTasks,task],addingFor:null,newLabel:"",newIcon:"📝"});
}
...
function buildHeader(){
  ...
  return el("div",{style:{background:"#1e293b",borderBottom:"1px solid #334155",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10}},
    el("div",null,
      el("div",{style:{fontSize:16,fontWeight:700}},"🎯 Kaushlaya's Tracker"),
    ...
    el("div",{style:{display:"flex",gap:4}},
      ...["today","calendar","stats"].map(v=>
        el("button",{className:"tab-btn"+(state.view===v?" active":""),onClick:()=>setState({view:v})},
          v==="today"?"📋 Today":v==="calendar"?"📅 Cal":"📊 Stats"
        )
      )
    )
  );
...
function buildReminder(){
  ...
  return el("div",{style:{background:"rgba(245,158,11,.1)",borderBottom:"1px solid rgba(245,158,11,.3)",padding:"8px 16px",fontSize:13,color:"#f59e0b"}},
    `⏰ ${rem} task${rem!==1?"s":""} remaining today — keep going!`
  );
}
...
function buildTaskRow(task,done,type,idx,onToggle){
  ...
  if(!task.fixed){
    const rb=el("button",{className:"remove-btn",onClick:e=>{e.stopPropagation();removeTask(type,task.id)}},"✕");
    row.appendChild(rb);
  }
  const cc=el("div",{className:"check-circle"+(done?(type==="weekly"?" checked-weekly":" checked-daily"):"")},done?"✓":"");
  row.appendChild(cc);
  row.appendChild(el("span",{style:{color:"#475569",cursor:"grab",fontSize:14}},"⠿"));
  return row;
}
...
function buildAddForm(type){
  ...
  const canB=el("button",{style:{background:"#334155",color:"#94a3b8",padding:"8px 10px",borderRadius:8,fontSize:13},onClick:()=>setState({addingFor:null,newLabel:"",newIcon:"📝"})},"✕");
...
function buildToday(){
  ...
  prog.appendChild(el("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:6,fontSize:13,color:"#94a3b8"}},
    ...
    el("span",{style:{color:"#fff",fontWeight:700}},`${done}/${total} — ${pct}%`)
  ));
  ...
  // Daily
  wrap.appendChild(el("div",{className:"section-label"},"Daily Tasks — drag to reorder"));
...
function buildCalendar(){
  ...
  nav.appendChild(el("button",{className:"nav-btn",onClick:()=>{const d=new Date(y,m-1);setState({cal:{y:d.getFullYear(),m:d.getMonth()}})}},"◀"));
  nav.appendChild(el("div",{style:{textAlign:"center"}},
    el("div",{style:{fontWeight:700}},monthName(y,m)),
    el("div",{style:{fontSize:11,color:isPM?"#22c55e":"#94a3b8"}},isPM?"🏆 Perfect Month!":pd+"/25 perfect days")
  ));
  nav.appendChild(el("button",{className:"nav-btn",onClick:()=>{const d=new Date(y,m+1);setState({cal:{y:d.getFullYear(),m:d.getMonth()}})}},"▶"));
  ...
  getWeeks(y,m).forEach(week=>{
    ...
    week.forEach(day=>{
      ...
      const cell=el("div",{className:cls},
        ...
        perf?el("span",{style:{fontSize:8}},"⭐"):null
      );
    ...
    if(pDays>0||isPW){
      wrap.appendChild(el("div",{style:{textAlign:"right",fontSize:10,marginBottom:4,color:isPW?"#22c55e":"#475569",fontWeight:isPW?"700":"400"}},
        isPW?`🏅 Perfect Week! (${pDays} days)`:`${pDays}/6 perfect days`
      ));
    }
  });
...
function buildStats(){
  ...
  [
    {e:"⭐",val:pd,label:"Perfect Days",color:"#22c55e"},
    {e:"🏆",val:isPM?"YES!":pd+"/25",label:"Perfect Month",color:isPM?"#22c55e":"#f59e0b"},
    {e:"📈",val:Math.round((pd/totalDays)*100)+"%",label:"Completion",color:"#3b82f6"},
  ].forEach(c=>{
  ...
  chartCard.appendChild(el("div",{style:{fontSize:13,fontWeight:600,color:"#94a3b8",marginBottom:10}},`Task Completion — ${monthName(y,m)}`));
...
function buildModal(){
  ...
  if(perf)hdrL.appendChild(el("div",{style:{color:"#22c55e",fontSize:12,fontWeight:600}},"⭐ Perfect Day!"));
  hdr.appendChild(hdrL);
  hdr.appendChild(el("button",{style:{background:"#334155",color:"#94a3b8",border:"none",borderRadius:8,padding:"4px 10px",fontSize:16,cursor:"pointer"},onClick:()=>setState({selDay:null})},"✕"));
  box.appendChild(hdr);
  ...
  state.dailyTasks.forEach(task=>{
    ...
    row.append(el("span",null,task.icon),
      ...
      el("div",{style:{width:18,height:18,borderRadius:"50%",border:`2px solid ${done?"#22c55e":"#475569"}`,
        background:done?"#22c55e":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:9,color:"#fff"}},done?"✓":"")
    );
    box.appendChild(row);
  });
  ...
  state.weeklyTasks.forEach(task=>{
    ...
    row.append(el("span",null,task.icon),
      ...
      el("div",{style:{width:18,height:18,borderRadius:"50%",border:`2px solid ${done?"#3b82f6":"#475569"}`,
        background:done?"#3b82f6":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:9,color:"#fff"}},done?"✓":"")
    );
    box.appendChild(row);
  });
